...
class RenjuGame:
    ...
    def _update_status_message(self):
        ...
        if self.game_state == GameState.PLAYING:
            ...
            p_type = "(H)" if self.player_types[self.current_player] == "human" else "(AI)"
            if self.move_count == 0 and self.current_player == BLACK:
                self.status_message = f"{p_name}{p_type} 回合 (請下天元)"
            else:
                self.status_message = f"{p_name}{p_type} 回合"
        elif self.game_state == GameState.PAUSED:
             p_name="黑方" if self.current_player==BLACK else "白方"; p_type="(H)" if self.player_types[self.current_player]=="human" else "(AI)"
             self.status_message = f"{p_name}{p_type} 暫停 (按鈕恢復)"
        elif self.game_state == GameState.BLACK_WINS:
             p_type = "(H)" if self.player_types[BLACK] == "human" else "(AI)"; self.status_message = f"黑方{p_type} 勝!"
        elif self.game_state == GameState.WHITE_WINS:
             p_type = "(H)" if self.player_types[WHITE] == "human" else "(AI)"; self.status_message = f"白方{p_type} 勝!"
        elif self.game_state == GameState.DRAW: self.status_message = "平局!"
    ...
    def pause_game(self):
        if self.game_state == GameState.PLAYING:
            ...
            self._update_status_message(); print("Game Paused")
    def resume_game(self):
        if self.game_state == GameState.PAUSED and self.pause_start_time is not None:
            ...
            self.game_state = GameState.PLAYING; self.pause_start_time = None; self._update_status_message(); print(f"Resumed (Paused {paused:.1f}s)")
        elif self.game_state == GameState.PAUSED:
            self.game_state = GameState.PLAYING; self.last_update_time = time.time(); self._update_status_message(); print("Warn: Resuming inconsistent pause.")
    ...
    def make_move(self, r, c):
        ...
        if not valid:
             pname="黑方" if player==BLACK else "白方"; ptype="(H)" if self.player_types[player]=="human" else "(AI)"
        ...
        if rules.check_win_condition_at(r, c, player, self.board):
            ...
            pname="黑方" if player==BLACK else "白方"; ptype="(H)" if self.player_types[player]=="human" else "(AI)"
            print(f"Win for {pname}{ptype} at ({r},{c})")
            self._update_status_message()
            if self.player_types[loser]=="ai":
