...
function _canPieceMechanicallyAttack(board, piece, fromX, fromY, toX, toY) {
    ...
    switch (piece.name) {
        ...
        case '象': case '相': return false; // Cannot attack king directly
        case '士': case '仕': return false; // Cannot attack king directly
        case '將': case '帥': return false; // Facing check handled in isKingInCheck
        case '炮': if (dx !== 0 && dy !== 0) return false; return countPiecesBetween(board, fromX, fromY, toX, toY) === 1; // Must jump exactly one piece to attack
...
function isValidMoveInternal(board, from, to, playerColor) {
    ...
    if (!piece || piece.color !== playerColor) return { valid: false, reason: "非指定顏色棋子" }; // 檢查顏色是否匹配傳入的 playerColor
...
// --- 導出 ---
module.exports = {
    isValidMove, // 標準模式接口 (檢查輪次)
    isValidMoveInternal, // 分析板模式接口 (不檢查輪次)
    isKingInCheck,
    isCheckmate,
    isStalemate,
    createBoardCopy
